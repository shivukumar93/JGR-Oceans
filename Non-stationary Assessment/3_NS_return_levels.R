...
setwd("G:/Lockdown/IJOC nons_revision/processing")

library(extRemes)
library(readxl)
library(coda)

combination <- read.csv("combination2.csv")
combination<-combination[,2:7]

best_combo<- read.csv("best_combo_prec.csv", header=TRUE)
best_combo<-best_combo[,2:358]

p_val <- read.csv("min_pval_prec.csv")
p_val<- p_val[,2:358]


comb_id<-c(358,359,360)

osc_cov<-read.csv('JJAS_COV.csv')
osc_cov<-osc_cov[1:104,]
...
{ 
  for(index in 2)
  {
    ...
    filename111<-paste('prec_ind_',id_ind[index],'.csv',sep="")
    data <- read.csv(filename111, header = TRUE)
    data<-data[,2:358]
    data<-cbind(data,osc_cov)
    ...
    if (p_val[index,point]>0.05)
    {
      ...
      output<-heidel.diag(fits$results,eps=0.05,pvalue = 0.5)
    ...
    file123<-paste('prec_para_ind_',point,'_ind_',index,'.RData')
